...
in {
  ...
  config = lib.mkIf cfg.enable (
    ...
    in {
      systemd = let
        serviceName =
          if config.system.impermanence.enable
          then "borg-backup-btrfs-subvolume"
          else "borg-backup-paths";
        serviceEntries = {
      ...
      in
        {
          ...
          in ["d ${homeDirectory}/mount/backup/ 0750 ${user} ${group} -"];
        }
        // serviceEntries;
