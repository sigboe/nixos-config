...
3. generate an access key (ssh key) for nix-secrets `ssh-keygen -t ed25519 -C "hostname"`
...
7. `sudo nixos-rebuild boot` (this will switch us from the -bootstrap ghost config to the config that has lanzaboote)
